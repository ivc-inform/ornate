...
import org.commonmark.node.{Block, Node, Text}
...
class CustomParser(builder: CustomParserBuilder) { // Cannot extend Parser because the constuctor is private
  val blockParserFactories = DocumentParser.calculateBlockParserFactories(builder._blockParserFactories, Set[Class[_ <: Block]]().asJava)
  ...
  def parse(input: String): Node = {
    val iipImpl = new InlineParserImpl(builder._delimiterProcessors)
    def wrap(i: InlineParser): InlineParserImpl = new InlineParserImpl(null) {
